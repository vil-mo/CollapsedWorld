...
func walk(direction : Vector2, delta : float):
	...
	var current_stats : FighterStats = entity.stats.current_stats
	
	if entity.velocity.length() > current_stats.speed:
		entity.velocity = entity.velocity.normalized() * (entity.velocity.length() - current_stats.hight_force_deceleration * delta)
	else:
		if direction == Vector2.ZERO:
			entity.velocity = entity.velocity.move_toward(Vector2.ZERO, delta * current_stats.speed / current_stats.walking_deceleration)
		else:
			entity.velocity = entity.velocity.move_toward(direction * current_stats.speed, delta * current_stats.speed / current_stats.walking_acceleration)
